...
describe('submissionCore.submitPrompt — synchronous busy (queue-race fix)', () => {
  ...
  it('removes the private voice marker before drop detection and agent input', async () => {
    ...
    // A key can arrive in the dashboard's PTY pacing window after the marker.
    // The marker remains protocol-only; it must never reach the agent or UI.
    submitPrompt(`voice${VOICE_TURN_MARKER} text`, makeDeps(gw))
    await Promise.resolve()
    await Promise.resolve()
