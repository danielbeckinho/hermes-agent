...
class GatewayKanbanWatchersMixin:
    ...
    async def _kanban_lifecycle_send_decision(
        ...
        text = (
            f"🗳️ {board_tag}Decision needed — {title}\n"
            f"Blocked: {reason}\n"
    ...
    async def _kanban_lifecycle_speak(self, adapter, task, ev) -> None:
        ...
        if ev.kind == "completed":
            phrase = f"{title} completed."
        elif ev.kind == "blocked":
            phrase = f"{title} needs your input."
        else:
            phrase = f"{title} needs a decision."
        try:
            await speak(phrase)
        except Exception:
            pass
                    ...
                    for ev in d["events"]:
                        ...
                        if kind == "completed":
                            ...
                            if payload_summary:
                                ...
                                handoff = f"\n{r}"
                            msg = (
                                f"✔ {board_tag}{tag}{title} done{handoff}"
                            )
                        elif kind == "blocked":
                            reason = ""
